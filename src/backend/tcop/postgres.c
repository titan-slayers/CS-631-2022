...
exec_simple_query(const char *query_string)
{
	...
	foreach(parsetree_item, parsetree_list)
	{
        ...
        // FD VIOLATION INSERTION CHECK
        
		char firstWordQuery[100];		// 's' will store the first word of the query
		int i=0;
		const char *inp_query = query_string;

	
		while(inp_query[i] != ' ')
		{
			firstWordQuery[i]=inp_query[i];
			i++;
		} 
		firstWordQuery[i] = '\0';
		tolower(firstWordQuery);

		if(strcmp("insert",firstWordQuery) == 0)		// if the first word of the query is "insert", only then we are checking for functional dependency violation
		{
			elog(NOTICE,"Insertion FD Violation Checking....");

			char tname[500];		// 'tname' will store the table name (extracted from the query) into which a row will be inserted
			i++;		//inp_query[i] == 'I'
			...
			tname[j] = '\0';
			if(strcmp(tname, "fd") != 0)	// to check that the row to be inserted is not a functional dependency, to be inserted in the 'fd' table
			{

			char values[500][50];						// 'values' stores column values to be inserted
			bool blank = false;
			int col_len; 				// 'col_len' stores the number of columns of the particular table
			char cols[500][50];			// 'cols' stores column names
			bzero(values, 500*50);
			bzero(cols, 500*50);

			if(inp_query[i] == ' ') // For Queries of the kind "INSERT INTO foo VALUES(3, 4, 5);"
				{
					blank = true;
					i++;
					while(inp_query[i] != '(')
					...
					sprintf(fd_query, "SELECT column_name FROM information_schema.columns WHERE table_name = '%s';",tname);
					ret = SPI_exec(fd_query, 0);
					proc = SPI_processed;
					// elog(NOTICE,"HEREEEEE %d",proc); 
					// elog(NOTICE,"hereeee %d",ret);
					if (ret > 0 && SPI_tuptable != NULL)
				    {
				        ...
				        for (j = 0; j < proc; j++)
				        {
				            ...
				            sprintf(cols[col_id], SPI_getvalue(tuple, tupdesc, 1));
				            col_id++;
				        }
				        col_len = proc;
				    }
			...
			else if(inp_query[i] == '(') // For Queries of the kind "INSERT INTO foo(a, b, c) VALUES(3, 4, 5);"
			{
				...
				// To retrieve the functional dependencies of the table from the 'fd' table
				sprintf(fd_query, "select det,dep from fd where table_name='%s';",tname);				
				...
				if (ret > 0 && SPI_tuptable != NULL)
			    {
			        TupleDesc tupdesc = SPI_tuptable->tupdesc;
			        SPITupleTable *tuptable = SPI_tuptable;
			        char buf_det[8192], buf_dep[8192];		// buf_det: the column name of the determinant column, and buf_dep: the column name of the dependent column
			        int i, j;
			        ...
			        for (j = 0; j < proc; j++)
			        {
			            ...
			            bzero(buf_det, 500); bzero(buf_dep, 500);
			            sprintf(buf_det, SPI_getvalue(tuple, tupdesc, 1));
			            
			            sprintf(buf_dep, SPI_getvalue(tuple, tupdesc, 2));
			            ...
			            for(i = 0; i<col_len; i++)
			            {
			            	if(strcmp(cols[i],buf_det) == 0) 		// column_name found in det of table
				            	break;
			            }
						

			            int dep_id;
			            for(dep_id = 0; dep_id<col_len; dep_id++)
			            {
			            	if(strcmp(cols[dep_id],buf_dep) == 0) 		// column_name found in dep of table
			            		break;
			            }
			            if(i!=col_len)		//column found in det
			            {
			            	
			            	int len = strlen(values[dep_id]);
			            	char *subString = values[dep_id]+1;		// 'substring' stores the value (to be inserted) of the dependent column
			            	if(values[dep_id][0]!='\'')
			            		subString--;
			            	else
			            	...
			            	// To retrieve the columns with values of the determinant column as the ones to be inserted by this insert query
							sprintf(fd_query, "select %s from %s where %s=%s",buf_dep, tname, buf_det, values[i]);
							
							ret = SPI_exec(fd_query, 0);
							int procx = SPI_processed;
							
							if(ret>1 && SPI_tuptable != NULL)
							{
								TupleDesc tupdesc = SPI_tuptable->tupdesc;
						        SPITupleTable *tuptable = SPI_tuptable;
						        ...
						        for (jx = 0; jx < procx; jx++)
						        {
						            ...
						            // If the value-to-be-inserted of the dependent column is not the same as the already-exisiting-value of the dependent column in the table,
						            // then there's a functional dependency violation, and thus an error is raised aborting the operation
						            if(strcmp(cck, subString)!=0)
						            {
						            	SPI_finish();
						            	ereport(ERROR,(errcode(ERRCODE_FUNCTIONAL_DEPENDENCY_VIOLATION),errmsg("Current Insertion is Aborted, ""Functional Dependency Violation: this value %s in '%s' has multiple values in '%s'", values[i], buf_det, buf_dep),errdetail_abort()));
										//break;
						            }
						        }
		...
		else if(strcmp("update",firstWordQuery) == 0)	
		{

			elog(NOTICE,"Updation FD Violation Checking....");
			char tname[500];      // 'tname' will store the table name (extracted from the query) into which a row will be inserted
			char setvalues[500][200];	   //setvalues is a 2d array which stores set attribute name with value
			int itset=0;
			char wherevalues[500];	
					//inp_query[i] == 'I'
					// roll 5
					//blue ver2
					//
			...
			tname[j] = '\0';
			elog(NOTICE, "TABLE NAME : %s", tname);

			i++;   

//FORMAT OF UPDATE QUERY
// UPDATE table_name    
// SET column1 = value1,   
// column2 = value2....,   
// columnN = valueN    
...
// comma followed by nospace 
			...
			while(inp_query[i] != ' ' && inp_query[i]!=';'){
				while(inp_query[i]!=' ' && inp_query[i]!=';'){
					...
					flag=false;
					// elog(NOTICE, "reached while2");
					char columnname[200];
					// char valuename[100];
					int itcol=0;
					// int itval=0;
					// elog(NOTICE, "still in while2");
					while(inp_query[i] != ' '){
						columnname[itcol++]=inp_query[i++];
					}
					// elog(NOTICE, "%s", columnname);
					// elog(NOTICE, "even now in while2");        
					// elog(NOTICE, "%d", i);
					columnname[itcol]=' ';
					i=i+3;
					itcol++;
					// elog(NOTICE,"%d", i);
					// elog(NOTICE, "even now in while2 2");
					while(inp_query[i] != ',' && inp_query[i] != ' ' && inp_query[i] != ';'){
						columnname[itcol++]=inp_query[i];
						i++;
					}
					// elog(NOTICE, "and again in while2");
					columnname[itcol]='\0';
					strcpy(setvalues[itset], columnname);
					// strcpy(setvalues[itset][1], valuename);
					itset++;
			...
			if(inp_query[i]==';')
			{
				...
				char dep[500];
				SPI_connect();
				sprintf(fd_query, "select * from fd where table_name = '%s';", tname);
				...
				if (ret > 0 && SPI_tuptable != NULL)
				    {
				        ...
				        int jx; 
						        for (jx = 0; jx < procx; jx++)
						        {
						            ...
						            bzero(det, 500);
									bzero(dep, 500);
									//bzero(fds[findex], 8192);
									//bzero(fds[findex+1],8192);
									//bzero(det_temp, 8192);
									//bzero(dep_temp, 8192);
						            sprintf(det, SPI_getvalue(tuple, tupdesc, 2));
									int it_l = 0;
									int it_r = 0;

									while(det[it_r]!='\0'){
										det_temp[it_l++]=det[it_r++];
									}
									det_temp[it_l] = '\0';
									sprintf(dep, SPI_getvalue(tuple, tupdesc, 3));

									it_l = 0, it_r = 0;

									while(dep[it_r]!='\0'){
										dep_temp[it_l++]=dep[it_r++];
									}
									dep_temp[it_l]='\0';

									
									strcpy(fds[findex],det_temp);
									strcpy(fds[findex+1],dep_temp);

									elog(NOTICE,"%s ",fds[findex]);
									elog(NOTICE,"%s \n",fds[findex+1]);
				...
				SPI_finish();
				elog(NOTICE,"%d\n",itset);
				char query_cols[500][500];
				for(i=0; i<itset; i++)
				{
					char* token = strtok(setvalues[i], " ");
				    strcpy(query_cols[i],token);
					elog(NOTICE,"%s\n",query_cols[i]);
				}

				for(i=0; i<findex; i+=2)
				{
					...
					for(j=0; j<itset; j++)
					{
						if(strcmp(fds[i],query_cols[j])==0)
						{
							contains_x = true;
						}
						if(strcmp(fds[i+1],query_cols[j])==0)
						{
							contains_y = true;
						}
					...
					if(contains_x && !contains_y){
						SPI_connect();
				    // To retrieve column names of the table
					sprintf(fd_query, "select count(distinct %s)  from %s ;",fds[i+1] ,tname);
				    ...
				    {    
						...
						for (int jx = 0; jx < procx; jx++)
						{  HeapTuple tuple = tuptable->vals[jx];
							sprintf(count, SPI_getvalue(tuple, tupdesc, 1));
							elog(NOTICE,"distinct count %c",count[0]);
							if(count[0]!='1' && count[0]!='0')
							{
								elog(NOTICE ,"CANT UPDATE");
								//ereport(ERROR,(errcode(ERRCODE_FUNCTIONAL_DEPENDENCY_VIOLATION),errmsg("Current Insertion is Aborted, ""Functional Dependency Violation: this value %s in '%s' has multiple values in '%s'", values[i], buf_det, buf_dep),errdetail_abort()));
				...
				i++;
				//reached where condition start
				char wherecond[500];
				int itwhere=0;
				while(inp_query[i] != ';'){
					wherecond[itwhere++] = inp_query[i++];
				}
				wherecond[itwhere]='\0';
				elog(NOTICE,"Where condition -  %s", wherecond);
				//entire where condition in query select * from tname where inp_query[i]---;
					

					SPI_connect();
				    // To retrieve column names of the table
					sprintf(fd_query, "select * from %s where %s;", tname, wherecond);
					//sprintf(fd_query2, "SELECT count(*) FROM information_schema.columns WHERE table_name =%s;", tname, wherecond);
					elog(NOTICE,"Executing the query : %s" ,fd_query);
					int ret = SPI_exec(fd_query, 0);
					int procx = SPI_processed;

	 // characer 2d array [rows=procx]
	 //col1 col2 col3
	 //col1 col2 col3
					...
					if (ret > 0 && SPI_tuptable != NULL)
				    {
						...
						int it_v=0;
						        for (jx = 0; jx < procx; jx++)
						        {
									...
									temp[it_t]=' ';
									it_t++;
									sprintf(cck, SPI_getvalue(tuple, tupdesc, 2));
									//elog(NOTICE,"cck id %s",cck );
									it_c=0;
									while(cck[it_c]!='\0'){
										temp[it_t++]=cck[it_c++];
									...
									sprintf(cck, SPI_getvalue(tuple, tupdesc, 3));
									//elog(NOTICE,"cck pid %s",cck );
									it_c=0;
									while(cck[it_c]!='\0'){
										temp[it_t++]=cck[it_c++];
									}
									...
									strcpy(validtup[it_v], temp);
									//elog(NOTICE, "extra-----%s %s",temp, validtup[it_v]);
									it_v++;
				   				 }
								 for(int x=0;x<procx;x++){
									elog(NOTICE, "Matched Tuple %d = %s", x, validtup[x]);
								 }
