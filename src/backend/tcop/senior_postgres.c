...
//int execq(const char *sql, int cnt);
...
exec_simple_query(const char *query_string)
{
	...
	foreach(parsetree_item, parsetree_list)
	{
		...
		char s[] = "";		// 's' will store the first word of the query
		int i;
		i = 0;
		const char *inp_query = query_string;
		while(inp_query[i] != ' ')
		{
			strncat(s, &inp_query[i], 1);
			i++;
		} 
		tolower(s);
		if(strcmp("insert",s) == 0)		// if the first word of the query is "insert", only then we are checking for functional dependency violation
		{
